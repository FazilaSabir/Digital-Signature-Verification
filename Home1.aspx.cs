...
namespace EService
{
    public partial class Home1 : System.Web.UI.Page
    {
        ...
        private ValidationResult itext_Verify(byte[] pdf)
        {
            ...
            foreach (string name in names)
            {
                ...
                if (signatureValid)
                {
                 ...
                 //   result.C_signer = iTextSharp.text.pdf.security.CertificateInfo.GetSubjectFields(pk.SigningCertificate).GetField("C");
                  //  result.CN_issuer = iTextSharp.text.pdf.security.CertificateInfo.GetIssuerFields(pk.SigningCertificate).GetField("CN");
                   // result.OU_issuer = iTextSharp.text.pdf.security.CertificateInfo.GetIssuerFields(pk.SigningCertificate).GetField("OU");
                  result.O_issuer = iTextSharp.text.pdf.security.CertificateInfo.GetIssuerFields(pk.SigningCertificate).GetField("O");

                    //   result.C_issuer = iTextSharp.text.pdf.security.CertificateInfo.GetIssuerFields(pk.SigningCertificate).GetField("C");
                    //   result.nr_serial =Int64.Parse(pk.SigningCertificate.SerialNumber);
                    // result.date = pk.SignDate;
                    // result.Issuer = pk.Location;
                    // result.VersionNumber = pk.Certificates.FirstOrDefault().Version;
        ...
        //private static void VerifySignatue(AcroFields fields, string name)
        //{
        //    Console.WriteLine("Signature covers whole document: " + fields.SignatureCoversWholeDocument(name));
        //    Console.Write("Document revision: " + fields.GetRevision(name) + " of " + fields.TotalRevisions);
        //    PdfPKCS7 pkcs7 = fields.VerifySignature(name);
        //    Console.WriteLine("Integrity check OK? " + pkcs7.Verify());
        //    Console.WriteLine("Sign Name" + pkcs7.Certificates.FirstOrDefault().GetIssuerAlternativeNames());
        //    Console.WriteLine("Sign Date" + pkcs7.SignDate);
        //    Console.WriteLine("Version:" + pkcs7.GetType());
        //}
        ...
        protected void btn_Verify_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    //using (Stream cs = Assembly.GetExecutingAssembly().GetManifestResourceStream(filename))
                    //{
                    ...
                    //byte[] checksum = sha.ComputeHash(docContent);
                    ...
                    gg.Rows.Add(new Object[] { filename, result.isvalid, KBs, result.CN_signer, result.O_issuer, hashedData });
                    //if (gg.Rows.Count > 0)
                    //{
                ...
                printreport.Visible = FileUpload1.HasFile;
